...
export const authoringExamples = [
	{
		id: 'motion-component',
		title: 'Motion elements in a keyed list',
		lab: '/motion-lab/state',
		summary: 'Motion creates one binding per element and installs its native exit transition.',
		note: 'The as prop selects a native HTML element. A shared layoutGroup connects local layout IDs. Motion options belong in motion; native attributes and CSS style stay on the component.',
...
<MotionConfig transition={{ duration: 0.24 }}>
  <ul>
	...
	},
	{
		...
		note: 'Create one binding per simultaneously mounted element. Motion durations are seconds; the small presence() helper uses milliseconds.',
		source: `<script lang="ts">
...
<style>
  ...
  p { min-height: 700px; padding: 24px; }
	...
	},
	{
		...
		note: 'Call during component initialization. The target getter follows bind:this; current starts false on the server. once stops observing after entry and resets when the target changes.',
		source: `<script lang="ts">
