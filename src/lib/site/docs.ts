		...
		sections: [
			...
			},
			{
				...
				text: [
					...
					'style accepts a native CSS string or a Motion style object. Object keys merge over motion.style keys, preserving MotionValues. An undefined object value clears the matching nested style; removing the key restores its nested fallback. A CSS string supplies native declarations alongside nested Motion styles. Omitting style or setting it to null leaves motion.style in effect. Motion owns the animated properties. Keep raw transform separate from decomposed x, y, rotate and scale values.',
